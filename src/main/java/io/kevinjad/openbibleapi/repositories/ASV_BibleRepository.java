...
public interface ASV_BibleRepository extends Repository<Verse,Integer> {
    Verse findVerseById(Integer id);

    @Query(value = "SELECT * FROM t_asv WHERE id BETWEEN ?1 AND ?2",nativeQuery = true)
    List<Verse> findListOfVerses(Integer id1, Integer id2);

    @Query(value = "SELECT * FROM t_asv WHERE t LIKE %:versePhrase%",nativeQuery = true)
